...
class BookingController extends Controller
{
    ...
    public function processAdd(Request $request)
    {
        ...
        try {
            ...
            if ($request->sales_order_type == 0) {
                ...
                // * buat jurnal
                $journal = JournalHelper::token($token)->make('Booking Full Book', $request->total_amount, ['hotel_account', 'hotel_cash_account']);
                $si = SalesInvoice::where('sales_invoice_token', $token)->first();
                $sales_invoice_id = $si->sales_invoice_id;
            }
            SalesOrder::create([
            ...
            if (!empty($journal)) {
                $journal->update(['transaction_journal_no' => $order->sales_order_no]);
            }
            foreach ($roomData as $roomval) {
                if (empty($price[$roomval])) {
                    $priced = $room->find($roomval)->price->first();
    ...
    public function processEdit(Request $request)
    {
        ...
        try {
            ...
            if ($request->sales_order_type == 0) {
                ...
                // * buat jurnal
                JournalHelper::token($token)->trsJournalNo($order->sales_order_no)->make('Booking Full Book', $request->total_amount, ['hotel_account', 'hotel_cash_account']);
                //
                $si = SalesInvoice::where('sales_invoice_token', $token)->first();
                $sales_invoice_id = $si->sales_invoice_id;
            }
    ...
    public function delete($sales_order_id)
    {
        try {
            ...
            if ($so->sales_order_status == 1) {
            }
            $so->delete();
            DB::commit();
            return redirect()->route('booking.index')->with(['msg' => 'Hapus Booking Kamar Berhasil', 'type' => 'success']);
        } catch (\Exception $e) {
            DB::rollBack();
            report($e);
            return redirect()->route('booking.index')->with(['msg' => 'Hapus Booking Kamar Gagal', 'type' => 'danger']);
        }
    }
}
