...
class PaymentReservationController extends Controller
{
    ...
    public function searchPurchasePayment()
    {
        ...
        $coresupplier = PurchaseInvoice::select('purchase_invoice.supplier_id', 'purchase_invoice.purchase_method', 'core_supplier.supplier_name', 'core_supplier.supplier_address', DB::raw("SUM(purchase_invoice.owing_amount) as total_owing_amount"))
        ->join('core_supplier', 'core_supplier.supplier_id', 'purchase_invoice.supplier_id')
        ->where('purchase_invoice.data_state', 0)
        ->where('purchase_invoice.purchase_method', 2)
    ...
    public function selectCustomerReservationPayment($sales_invoice_reservation_id)
    {
        $reservation = SalesInvoiceReservation::select('*')
        ...
        return view('content.PaymentReservation.AddPaymentReservation', compact('reservation','purchasepaymentelements','payment_method_list','supplier', 'account'));
    }
    ...
    {   
        ...
        for ($i=1; $i <= $request->total_invoice ; $i++) {
            if ($request['sales_invoice_reservation_id_'.$i] != null) {
                ...
                // $payable_amount += $purchase_invoice['owing_amount'];
            }
        }
        ...
        for ($i=1; $i <= $request->total_invoice ; $i++) {
            if ($request['sales_invoice_reservation_id_'.$i] != null) {
                ...
                $purchase_invoice = SalesInvoiceReservation::where('sales_invoice_reservation_id', $request['sales_invoice_reservation_id'])
                ->first();

                $table                  = SalesInvoiceReservation::findOrFail($request['sales_invoice_reservation_id']);
                $table->owing_amount     = $request['rounding_amount'];
        ...
        //             $debit_amount = $purchasepayment['payment_amount'] - $request->subtraction_amount;
        //             $credit_amount = 0;
        //         } else {
        //             $debit_amount = 0;
        //             $credit_amount = $purchasepayment['payment_amount'] - $request->subtraction_amount;
        //         }
        //         $journal_debit = array(
        //             'company_id'                    => Auth::user()->company_id,
        //             'journal_voucher_id'            => $journal_voucher_id['journal_voucher_id'],
        //             'account_id'                    => $account_id,
        //             'journal_voucher_amount'        => $purchasepayment['payment_amount'] - $request->subtraction_amount,
    ...
    public function deletePurchasePayment($payment_id)
    {
        ...
        foreach ($purchasepaymentitem as $key => $val) {
            ...
            $tablePurchaseInvoice               = PurchaseInvoice::findOrFail($val['sales_invoice_reservation_id']);
