    ...
    <script>
                                ...
                                $.ajax({
                                    ...
                                    success: function(margin) {
                                        if (margin != '') {
                                            ...
                                            var price_new = parseInt(cost_new) + ((
                                                parseInt(cost_new) * margin
                                            ) / 100);
        ...
        function processAddArrayPurchaseInvoice() {
            ...
            if (quantity == '' || quantity == 0) {
                alert('Masukan Jumlah Barang yang dibeli!');
                $('#quantity').focus();
                return 0;
            }
            $.ajax({
        ...
        $(document).ready(function() {
            ...
            $('#purchase_invoice_date').change(function() {
                ...
                if (due_date < date_invoice) {
                    ...
                    setTimeout(() => {
                        function_elements_add('purchase_invoice_due_date', moment(this.value)
                            .format('YYYY-MM-DD'));
                    }, 100);
                }
                $('#purchase_invoice_due_date').attr('min', moment(this.value).format('YYYY-MM-DD'));
                $('#purchase_invoice_due_day').val(moment($('#purchase_invoice_due_date').val()).diff(
                    moment($('#purchase_invoice_date').val()), 'days'));
                setTimeout(() => {
                    function_elements_add('purchase_invoice_date', moment(this.value).format(
                        'YYYY-MM-DD'));
                }, 100);
            });
            ...
            $('#purchase_invoice_due_date').attr('min', moment($('#purchase_invoice_date').val()).format(
                'YYYY-MM-DD'));
            var diff = moment($('#purchase_invoice_due_date').val()).diff(moment($('#purchase_invoice_date').val()),
                'days');
            if (diff < 0) {
                $('#purchase_invoice_due_date').val(moment($('#purchase_invoice_date').val()).format('YYYY-MM-DD'));
                setTimeout(() => {
                    function_elements_add('purchase_invoice_due_date', moment($('#purchase_invoice_date')
            ...
            $('#detailModal').on('show.bs.modal', function(event) {
                ...
                for (var key in arr) {
                    var value = arr[key];
                    $(this).find('.row-body').append("<div class='col-3'>" + key +
                        "</div><div class='col-auto'>:</div><div class='col-8'>" + value + "</div>");
                }
            })
        });
        function check() {
                    ...
                    <div class="row form-group">
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <input style="text-align: left" class="form-control input-bb" name="margin_percentage"
                                    id="margin_percentage" type="text" autocomplete="off" value="" />
                                <input style="text-align: left" class="form-control input-bb" name="margin_percentage_old"
                                    id="margin_percentage_old" type="text" autocomplete="off" value="" hidden />
                            </div>
                        </div>
                    </div>
            ...
            <div class="card border border-dark">
                <div class="card-header border-dark bg-dark">
                    ...
                    <div class="float-right">
                        <button onclick="location.href='{{ route('pi.index') }}'" name="Find"
                            class="btn btn-sm btn-info" title="Back"><i class="fa fa-angle-left"></i> Kembali</button>
                    </div>
                </div>
                <form id="form-invoice" method="post" action="{{ route('pi.process-add') }}"
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input class="form-control input-bb" name="purchase_invoice_date"
                                        ...
                                        value="{{ $datases['purchase_invoice_date'] ?? '' ? $datases['purchase_invoice_date'] ?? '' : date('Y-m-d') }}" />
                                </div>
                            </div>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input class="form-control input-bb" name="purchase_invoice_due_date"
                                            ...
                                            autocomplete="off" onchange="function_elements_add(this.name, this.value)"
                                            value="{{ $datases['purchase_invoice_due_date'] ?? '' ? $datases['purchase_invoice_due_date'] ?? '' : date('Y-m-d') }}" />
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input style="width: 100%" class="form-control input-bb"
                                            ...
                                            autocomplete="off" onchange="function_elements_add(this.name, this.value)"
                                            value="{{ $datases['purchase_invoice_due_day'] ?? '' ? $datases['purchase_invoice_due_day'] ?? '' : 0 }}" />
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="card-footer text-muted">
                        <div class="form-actions float-right">
                            ...
                            <button type="button" id="add_purchase_item" class="btn btn-success" title="Add">
                                Tambah</button>
                        </div>
                    </div>
            </div>
    ...
    <div class="card border border-dark">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-advance table-hover">
                        ...
                        <tbody>
                            ...
                            @if (empty($arraydatases))
                                <tr>
                                    <th colspan='6' style='text-align  : center !important;'>Data Kosong</th>
                                </tr>
                            @else
                                @foreach ($arraydatases as $key => $val)
                                    <tr>
                                        <td style='text-align  : left !important;'>{{ $val['item_name'] }} <input
                                                type="hidden" name="item[{{ $i }}][item_id]"
                                                value="{{ $val['item_id'] }}" /><input type="hidden"
                                                name="item[{{ $i }}][merchant_id]"
                                                value="{{ $val['merchant_id'] }}" /><input type="hidden"
                                                name="item[{{ $i }}][item_category_id]"
                                                value="{{ $val['item_category_id'] }}" /></td>
                                        <td style='text-align  : right !important;'>{{ $val['quantity'] }}
                                            {{ $val['item_unit_name'] }}<input type="hidden"
                                                name="item[{{ $i }}][quantity]"
                                                value="{{ $val['quantity'] }}" /> <input type="hidden"
                                                name="item[{{ $i }}][item_unit_id]"
                                                value="{{ $val['item_unit_id'] }}" /> </td>
                                        <td style='text-align  : right !important;'>
                                            {{ number_format($val['item_unit_cost'], 2, ',', '.') }}<input type="hidden"
                                                name="item[{{ $i }}][item_unit_cost]"
                                                value="{{ $val['item_unit_cost'] }}" /> </td>
                                        <td style='text-align  : right !important;'>
                                            {{ number_format($val['subtotal_amount_after_discount'], 2, ',', '.') }}<input
                                                type="hidden" name="item[{{ $i }}][subtotal_amount]"
                                                value="{{ $val['subtotal_amount_after_discount'] }}" /></td>
                                        <td style='text-align  : right !important;'>
                                            {{ date('d-m-Y', strtotime($val['item_expired_date'])) }}<input
                                                name="item[{{ $i }}][item_expired_date]" hidden type="date"
                                                value="{{ $val['item_expired_date'] }}" /></td>
                                        <td style='text-align  : center'>
                                            <a href="{{ route('pi.delete-array', ['record_id' => $key]) }}"
                                                name='Reset' class='btn btn-danger btn-sm'
                                                onclick="return confirm('Apakah Anda Yakin Ingin Menghapus Data Ini ?')">Hapus</a>
                                            <a name='Detail' class='btn btn-warning btn-sm' data-toggle="modal"
                                                data-target="#detailModal" data-id="{{ $key }}"> Detail</a>
                                        </td>
                                    </tr>
                                    @php
        ...
        <div class="card-footer text-muted">
            <div class="form-actions float-right">
                ...
                <button type="button" name="Save" id="save-transaction" class="btn btn-success"
                    onclick="$(this).prop('disabled',true);check();" title="Save"><i class="fa fa-check"></i>
                    Simpan</button>
            </div>
        </div>
        </form>
